...
<div class="container">
	...
	<div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Lịch sử mua hàng</h1>
        </div>
    </div>
                <!-- /.row -->
				<div class="row">
							...
							<?php
								...
								while($hrow=mysqli_fetch_array($h)){
									?>
										<tr>
											<td class="hidden"></td>
											<td><?php echo date("d/m/Y - h:i A", strtotime($hrow['sales_date']));?></td>
											<td><?php echo number_format($hrow['sales_total'],2); ?></td> 
											<!-- so 2 la so chu so xuat hien phia sau gia ban -->
											<td>
												<a href="#detail<?php echo $hrow['salesid']; ?>" data-toggle="modal" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-fullscreen"></span> Xem chi tiết</a>
